...
function Products() {
  ...
  // Filter products by category + subcategory - FIXED
  const filteredProducts = products.filter((p) => {
    ...
    const matchCategory = selectedCategory === "all" || productCategoryId === selectedCategoryId;
    
    // Check if subcategory matches (only if a subcategory is selected)
    let matchSubCategory = true;
    if (selectedSubCategory !== null) {
      const productSubCategoryId = String(p.subCategoryId);
      const selectedSubCategoryId = String(selectedSubCategory);
      matchSubCategory = productSubCategoryId === selectedSubCategoryId;
    }
  ...
  // Filter subcategories by search and by selected category
  const filteredSubCategories = searchTerm
    ? subCategories.filter(sub => 
        sub.name?.toLowerCase().includes(searchTerm.toLowerCase()) && 
        String(sub.categoryId) === String(selectedCategory)
      )
    : subCategories.filter(sub => String(sub.categoryId) === String(selectedCategory));
            ...
            <div className="flex items-center gap-4 justify-center">
              ...
              <div className="flex gap-3 md:gap-4">
                {filteredSubCategories.length > 0 ? (
                  ...
                  <p className="text-gray-400">No subcategories available for this category</p>
                )}
              </div>
              <button
                onClick={() =>
                  setSubCatIndex(
                    Math.min(
                      filteredSubCategories.length - visibleCount,
                      subCatIndex + 1
                    )
                  )
                }
